...
int mean(); // creates function mean
int factorial(); // creates function factorial
int fileF(); // creates function file calculation
...
int main() 
{
    ...
    while (X)  // start of while loop where you can choose operation functions to run
    {
        ...
        case '[': mean(); // when a user enters '[' run the function mean
            break;  // break code
        case 'f': fileF(); // when a user enters 'f' run the function file
            break; // break code
...
void calculator_operations()
{
    ...
    printf("Enter [ symbol for Mean \n"); // prints that pressing [ launches mean operations
    printf("Enter f symbol for file \n"); // prints that pressing f launches file operations
    ...
    printf("This program was updated %i/%i/%.2i.\n", today.month, today.day, today.year % 100);
}
...
{
    int a, b, d = 0.0; // declares a and b, declaes and defines d as 0
...
int mean() // Start array operation function
{
...
/* This is the file copy operation function
can copy answers from one file to another*/

void copy_files(FILE* in, FILE* out) // makes a function to copy files
{
    int c; //declares c
    while ((c = getc(in)) != EOF) //starts a while loop that gets characters until the end of the file
        putc(c, out); // puts the copied characters into the new file
}

int fileF(int argc, char* argv[]) // starts the function for file operation
{
    FILE* in, * out; // makes file in and out
    errno_t err; // makes an error to use if needed

    if (argc != 3) // start of if statment if argc isn't the same as 3
    {
        fprintf(stderr, "\nFile has been copied.\n"); // print statment pick teo files
        return 1; // end if statment
    }

    if ((err = fopen_s(&in, argv[1], "r")) != 0) // if statment where if err = fopen_s(&in, argv[1], "r"
    {
        fprintf(stderr, "Can't read from input file %s.\n", argv[1]); // print statment can't read input file
        return 2; // end if statment
    }

    if ((err = fopen_s(&out, argv[2], "w")) != 0) //if statment if err = fopen_s(&out, argv[2], "w"
    {
        fprintf(stderr, "Can't write to output file %s.\n", argv[2]); //print can't write output file
        return 3; // end if statment
    }

    copy_files(in, out); // call copy_files function with in and out as varables
    printf("File has been copied.\n"); // print files have been copied

    fclose(in); // close in text file
    fclose(out); //close out text file
}